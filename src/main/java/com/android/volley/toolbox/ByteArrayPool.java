...
/**
 * byte[]的回收池，用于byte[]的回收和再利用，减少了内存的分配和回收。
 ...
 * <p>
 * A good candidate（候选） user for this class is something like an I/O system that uses large temporary
 ...
 * 数组的大小不用介意，超出所需大小的buffer数组，超出部分会复用一个可回收的buffer
 * <p>
 * If there is not a suitably-sized buffer in its recycling pool when a buffer is requested, this
 * class will allocate（分配） a new buffer and return it.
 * <p>
